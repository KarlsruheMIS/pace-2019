...
struct NodeData {
    ...
    NodeWeight weight:48; // save a little bit of memory
...
public:
        ghost_node_communication() : m_iteration_counter(0), m_first_send(true) {
        ...
        void init( ) {
                m_num_adjacent = 0;
                for( PEID peID = 0; peID < m_adjacent_processors.size(); peID++) {
        ...
        inline 
        void set_skip_limit( unsigned long skip_limit ) {
                m_skip_limit = skip_limit;
        }

        inline 
        void set_desired_rounds( unsigned long desired_rounds) {
        ...
        PEID getNumberOfAdjacentPEs() {
                PEID counter = 0;
                for( unsigned long peID = 0; peID < m_adjacent_processors.size(); peID++) {
...
private:
        ...
        unsigned long m_skip_limit; 
        bool m_first_send;

        unsigned long m_send_iteration;
        unsigned long m_recv_iteration;

        unsigned long m_send_tag;
        unsigned long m_recv_tag;

        unsigned long m_desired_rounds;
        ...
        std::vector< MPI::Request* >          m_isend_requests;
};
...
public:
		...
		m_cur_degree                 = 0;
                ...
                if( update_comm_rounds ) {
                        ...
                        m_comm_rounds = std::max(m_comm_rounds, 8UL);
	...
	NodeID get_max_degree() {
        ...
        /* ============================================================= */
        void init_balance_management( PartitionConfig & config );
        ...
        static void set_comm_rounds(unsigned long comm_rounds); 
        static void set_comm_rounds_up(unsigned long comm_rounds); 
...
private:
        ...
        static unsigned long m_comm_rounds; // global number of edges
        static unsigned long m_comm_rounds_up; // global number of edges
...
inline void ghost_node_communication::update_ghost_node_data( bool check_iteration_counter ) {
        ...
        if( m_first_send ) {
                for( PEID peID = 0; peID < m_size; peID++) {
                        if( m_adjacent_processors[peID] ) {
                                ...
                                *request = MPI::COMM_WORLD.Isend( &(*m_send_buffers_ptr)[peID][0], 
        ...
        for( PEID peID = 0; peID < (*m_send_buffers_ptr).size(); peID++) {
                if( m_adjacent_processors[peID] ) {
                        //now we have to send a message
                        if( (*m_send_buffers_ptr)[peID].size() == 0 ){
                                // length 1 encode no message
...
void ghost_node_communication::receive_messages_of_neighbors() {
        ...
        while( counter < m_num_adjacent ) {
                ...
                MPI::Status st;
                while(MPI::COMM_WORLD.Iprobe(MPI::ANY_SOURCE,m_recv_tag,st)) {
                        unsigned long message_length = st.Get_count(MPI_UNSIGNED_LONG);
                        std::vector<NodeID> message; message.resize(message_length);

                        MPI::COMM_WORLD.Recv( &message[0], message_length, MPI_UNSIGNED_LONG, st.Get_source(), m_recv_tag); 
                        counter++;
...
inline void ghost_node_communication::update_ghost_node_data_global() {
        ...
        for( PEID peID = 0; peID < send_buffers.size(); peID++) {
                if( m_adjacent_processors[peID] ) {
                        ...
                        MPI::COMM_WORLD.Isend( &send_buffers[peID][0], 
                                                send_buffers[peID].size(), MPI_UNSIGNED_LONG, peID, peID+3*m_size);
                }
        }
        ...
        while( counter < m_num_adjacent ) {
                // wait for incomming message of an adjacent processor
                MPI::Status st;
                unsigned int tag = m_rank+3*m_size;
