...
public:
        ...
        void generate(PartitionConfig & config, parallel_graph_access & G) {
                ...
                if(global_rank < sq) {
                        ...
                        for( PEID peID = 0; peID < (PEID)size; peID++) {
                                ...
                                MPI::COMM_WORLD.Isend( &messages[peID][0], messages[peID].size(), MPI_UNSIGNED_LONG, peID, 0);
                        }

                }

                MPI::COMM_WORLD.Bcast(&nmbEdges,  1, MPI_UNSIGNED_LONG, 0);

                PEID peID = global_rank;
                unsigned long from           = peID     * ceil(nmbNodes / (double)size);
                unsigned long to             = (peID+1) * ceil(nmbNodes / (double)size) - 1;
                ...
                while( counter < (PEID)(sq )) {
                        ...
                        while(MPI::COMM_WORLD.Iprobe(MPI::ANY_SOURCE,0,st)) {
                                ...
                                MPI::COMM_WORLD.Recv( &incmessage[0], message_length, MPI_UNSIGNED_LONG, st.Get_source(), 0); 
                                counter++;

                                // now integrate the changes
                                if( incmessage[0] == std::numeric_limits< NodeID >::max()) continue; // nothing to do

                                for( unsigned long i = 0; i < message_length; ) {
