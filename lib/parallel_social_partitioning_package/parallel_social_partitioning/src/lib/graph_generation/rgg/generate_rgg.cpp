...
public:
  ...
  {
    ASSERT_BETWEEN( 0, idx, points_->size() - 1 );
    return (*points_)[idx];
  }
  ...
  {
    ASSERT_BETWEEN( 0, i, raster_count() );
    return offsets_[i];
  }
...
private:
  ...
  void point_xy_to_raster_xy(long x, long y, long *rx, long *ry) const
  {
    ...
    ASSERT_BETWEEN( x_off, x, x_off + x_len );
    ASSERT_BETWEEN( y_off, y, y_off + y_len );
    *rx = (x - x_off) / radius_;
    *ry = (y - y_off) / radius_;
    ASSERT_BETWEEN( 0, *rx, raster_x - 1 );
    ASSERT_BETWEEN( 0, *ry, raster_y - 1 );
  }
  ...
  void raster_xy_to_raster_id(long x, long y, long *i) const
  {
    ...
    *i = x + y * raster_x;
    ...
    ASSERT_BETWEEN( 0, *i, raster_x * raster_y - 1 );
  }
  ...
  void BuildRaster()
  {
    ...
    for (long i = 0, iend = points_->size(); i < iend; ++i) {
      ...
      ASSERT_GEQ( x, (*coordinate_splitters_)[process_x_] - radius_ );
      ASSERT_LT( x, (*coordinate_splitters_)[process_x_ + 1] + radius_ );
      ASSERT_GEQ( y, (*coordinate_splitters_)[process_y_] - radius_ );
      ASSERT_LT( y, (*coordinate_splitters_)[process_y_ + 1] + radius_ );
    }
#endif
    ...
    */
    ...
    //std::cout <<  "C"  << std::endl;
    ASSERT_EQ( offsets_.size(), raster_count + 1 );
    comm_->Barrier();
    if (rank_ == 0)
      printf("    Took %.2f s\n", timer.elapsed());
  }
...
#ifndef NDEBUG
  if (edges.size() > 0) {
    ASSERT_NEQ( edges[0].tail, edges[0].head );
    ASSERT_BETWEEN( vertex_splitters_[rank_], edges[0].tail,
                    vertex_splitters_[rank_ + 1] - 1);
  }
  for (long i = 1, iend = edges.size(); i < iend; ++i) {
    ASSERT_BETWEEN( vertex_splitters_[rank_], edges[i].tail,
                    vertex_splitters_[rank_ + 1] - 1);
    if (not (edges[i - 1].tail != edges[i].tail or edges[i - 1].head != edges[i].head)) { //XXX
      printf("i = %d, (%d, %d) == (%d, %d)\n", i,
             edges[i-1].tail, edges[i-1].head, edges[i].tail, edges[i].head);
    }
    ASSERT_NEQ( edges[i].tail, edges[i].head );
    ASSERT_TRUE( edges[i - 1].tail != edges[i].tail or edges[i - 1].head != edges[i].head );
  }
#endif
...
#ifndef NDEBUG
      ASSERT_GEQ( x, 0 );
      ASSERT_BETWEEN( coordinate_splitters_[px] - int_distance_, x,
                      coordinate_splitters_[px + 1] + int_distance_ - 1 );
      ASSERT_GEQ( y, 0 );
      ASSERT_BETWEEN( coordinate_splitters_[py] - int_distance_, y,
                      coordinate_splitters_[py + 1] + int_distance_ - 1 );
#endif
    ...
    for (long dx = -1; dx <= 1; ++dx) {
      for (long dy = -1; dy <= 1; ++dy) {
        ...
        for (long a = raster.raster_begin(i), aend = raster.raster_begin(i + 1); a < aend; ++a) {
          ...
          ASSERT_BETWEEN( vertex_splitters[rank_], std::tr1::get<2>(p1), vertex_splitters[rank_ + 1] - 1 );
          for (long b = raster.raster_begin(j), bend = raster.raster_begin(j + 1); b < bend; ++b) {
            //ASSERT_LEQ( edges->size(), 2 * points->size() * expected_average_degree );
