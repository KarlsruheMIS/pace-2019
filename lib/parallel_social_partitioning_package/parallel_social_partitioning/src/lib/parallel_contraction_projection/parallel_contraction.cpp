...
void parallel_contraction::contract_to_distributed_quotient( PartitionConfig & config, 
        ...
        compute_label_mapping( G, number_of_distinct_labels, label_mapping);
        ...
        get_nodes_to_cnodes_ghost_nodes( G );   
        ...
        m_out_messages.shrink_to_fit();
        m_send_buffers.resize(0); 
        m_send_buffers.shrink_to_fit(); 

        redistribute_hased_graph_and_build_graph_locally( hG, node_weights, number_of_distinct_labels, Q );

        update_ghost_nodes_weights( Q );

}

void parallel_contraction::compute_label_mapping( parallel_graph_access & G, 
        ...
        for( unsigned long peID = 0; peID < size; peID++) {
        ...
        for( unsigned i = 0; i < m_messages[rank].size(); i++) {
        ...
        while( counter < size - 1) {
                ...
                MPI::Status st;
                while(MPI::COMM_WORLD.Iprobe(MPI::ANY_SOURCE,rank+4*size,st)) {
                        unsigned long message_length = st.Get_count(MPI_UNSIGNED_LONG);
                        std::vector<NodeID> incmessage; incmessage.resize(message_length);

                        MPI::COMM_WORLD.Recv( &incmessage[0], message_length, MPI_UNSIGNED_LONG, st.Get_source(), rank+4*size); 
                        counter++;
        ...
        MPI::COMM_WORLD.Scan(&local_num_labels, &prefix_sum, 1, MPI_UNSIGNED_LONG, MPI_SUM); 

        global_num_distinct_ids = prefix_sum;
        // Broadcast global number of ids
        MPI::COMM_WORLD.Bcast(&global_num_distinct_ids, 1, MPI_UNSIGNED_LONG, size-1); 
        ...
        for( unsigned long i = 0; i < local_labels.size(); i++) {
        ...
        for( unsigned long peID = 0; peID < size; peID++) {
                ...
                for( unsigned long i = 0; i < inc_messages[peID].size(); i++) {
        ...
        for( PEID peID = 0; peID < size; peID++) {
                if( peID != rank ) {
                        MPI::COMM_WORLD.Isend( &m_out_messages[peID][0], 
        ...
        for( unsigned long i = 0; i < m_messages[rank].size(); i++) {
        ...
        while( counter < size - 1) {
                // wait for incomming message of an adjacent processor
                MPI::Status st;
                while(MPI::COMM_WORLD.Iprobe(MPI::ANY_SOURCE,rank+5*size,st)) {
                        unsigned long message_length = st.Get_count(MPI_UNSIGNED_LONG);
                        std::vector<NodeID> incmessage; incmessage.resize(message_length);

                        MPI::COMM_WORLD.Recv( &incmessage[0], message_length, MPI_UNSIGNED_LONG, st.Get_source(), rank+5*size); 
                        counter++;
...
void parallel_contraction::get_nodes_to_cnodes_ghost_nodes( parallel_graph_access & G ) {
        PEID size = MPI::COMM_WORLD.Get_size();
        PEID rank = MPI::COMM_WORLD.Get_rank();
        ...
        for( PEID peID = 0; peID < m_send_buffers.size(); peID++) {
                if( G.is_adjacent_PE(peID) ) {
                        ...
                        MPI::COMM_WORLD.Isend( &m_send_buffers[peID][0], 
                                                m_send_buffers[peID].size(), MPI_UNSIGNED_LONG, peID, peID+6*size);
                }
        }
        ...
        while( counter < num_adjacent ) {
                // wait for incomming message of an adjacent processor
                MPI::Status st;
                while(MPI::COMM_WORLD.Iprobe(MPI::ANY_SOURCE,rank+6*size,st)) {
                        unsigned long message_length = st.Get_count(MPI_UNSIGNED_LONG);
                        std::vector<NodeID> message; message.resize(message_length);

                        MPI::COMM_WORLD.Recv( &message[0], message_length, MPI_UNSIGNED_LONG, st.Get_source(), rank+6*size); 
                        counter++;
...
void parallel_contraction::redistribute_hased_graph_and_build_graph_locally( hashed_graph &  hG, 
                                                                             std::unordered_map< NodeID, NodeWeight > & node_weights,
        ...
        if( m_messages[ rank ].size() != 0 ) {
                for( unsigned long i = 0; i < m_messages[rank].size()-2; i+=3) {
                        hashed_edge he;
        ...
        while( counter < size - 1) {
                // wait for incomming message of an adjacent processor
                MPI::Status st;
                while(MPI::COMM_WORLD.Iprobe(MPI::ANY_SOURCE,rank+7*size,st)) {
                        unsigned long message_length = st.Get_count(MPI_UNSIGNED_LONG);
                        std::vector<NodeID> incmessage; incmessage.resize(message_length);

                        MPI::COMM_WORLD.Recv( &incmessage[0], message_length, MPI_UNSIGNED_LONG, st.Get_source(), rank+7*size); 
                        counter++;

                        // now integrate the changes
                        if( incmessage[0] == std::numeric_limits< NodeID >::max()) continue; // nothing to do

                        for( unsigned long i = 0; i < incmessage.size()-2; i+=3) {
                                hashed_edge he;
                                he.k = number_of_cnodes;
        ...
        unsigned long from = rank     * ceil(number_of_cnodes / (double)size);
        unsigned long to   = (rank+1) * ceil(number_of_cnodes / (double)size) - 1;
        // handle the case where we dont have local edges
        from = std::min(from, number_of_cnodes);
        to   = std::min(to, number_of_cnodes - 1);
        unsigned long local_num_cnodes = to - from + 1;
        ...
        long global_edges = 0;
        MPI::COMM_WORLD.Allreduce(&edge_counter, &global_edges, 1, MPI_UNSIGNED_LONG, MPI_SUM);
        ...
        while( counter < size - 1) {
                // wait for incomming message of an adjacent processor
                MPI::Status st;
                while(MPI::COMM_WORLD.Iprobe(MPI::ANY_SOURCE,rank+8*size,st)) {
                        unsigned long message_length = st.Get_count(MPI_UNSIGNED_LONG);
                        std::vector<NodeID> incmessage; incmessage.resize(message_length);

                        MPI::COMM_WORLD.Recv( &incmessage[0], message_length, MPI_UNSIGNED_LONG, st.Get_source(), rank+8*size); 
                        counter++;
...
void parallel_contraction::update_ghost_nodes_weights( parallel_graph_access & G ) {
        PEID size = MPI::COMM_WORLD.Get_size();
        PEID rank = MPI::COMM_WORLD.Get_rank();
        ...
        for( PEID peID = 0; peID < m_send_buffers.size(); peID++) {
                if( G.is_adjacent_PE(peID) ) {
                        ...
                        MPI::COMM_WORLD.Isend( &m_send_buffers[peID][0], 
                                                m_send_buffers[peID].size(), MPI_UNSIGNED_LONG, peID, peID+9*size);
                }
        }
        ...
        while( counter < G.getNumberOfAdjacentPEs()) {
                // wait for incomming message of an adjacent processor
                MPI::Status st;
                while(MPI::COMM_WORLD.Iprobe(MPI::ANY_SOURCE,rank+9*size,st)) {
                        unsigned long message_length = st.Get_count(MPI_UNSIGNED_LONG);
                        std::vector<NodeID> message; message.resize(message_length);

                        MPI::COMM_WORLD.Recv( &message[0], message_length, MPI_UNSIGNED_LONG, st.Get_source(), rank+9*size); 
                        counter++;
