...
public:
        ...
        void contract_to_distributed_quotient( PartitionConfig & config, 
                                               parallel_graph_access & G, 
                                               parallel_graph_access & Q);
private:
        // compute mapping of labels id into contiguous intervall [0, ...., num_lables)
        void compute_label_mapping( parallel_graph_access & G, 
                                    NodeID & global_num_distinct_ids,
                                    std::unordered_map< NodeID, NodeID > & label_mapping);

        void get_nodes_to_cnodes_ghost_nodes( parallel_graph_access & G );   
	...
	void build_quotient_graph_locally( parallel_graph_access & G, 
        ...
        void redistribute_hased_graph_and_build_graph_locally( hashed_graph &  hG, 
                                                               std::unordered_map< NodeID, NodeWeight > & node_weights,
                                                               NodeID number_of_cnodes,
                                                               parallel_graph_access & Q);

        void update_ghost_nodes_weights( parallel_graph_access & G ); 
