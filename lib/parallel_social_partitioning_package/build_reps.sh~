...
scons program=parallel_label_compress variant=optimized -j 4 -c
scons program=parallel_label_compress variant=optimized -j 4 
cd ..
cd ..

mkdir deploy
cp parallel_social_partitioning/src/optimized/parallel_label_compress deploy/
