...
is run on the kernel, \texttt{BnR-LS}\comment[id=DH]{I find these names
  confusing, why not \mbox{LSBnR} and BnR? The original BnR doesn't use local search} applies reductions and then runs the branch-and-reduce algorithm on the kernel (no local search is used to improve an initial bound), \texttt{FullA} is the full algorithm as described above.
